...
public class ForumSupMTIOujdaView extends Window implements Bindable {
	...
	private void updateUserRechercheState(Boolean state, int index) {
		try {
			...
			Image acceptedUser = Image.load(ForumSupMTIOujdaView.class.getResource("/ma/supmti/forum/view/img/pre.png"));
			Image canceledUser = Image.load(ForumSupMTIOujdaView.class.getResource("/ma/supmti/forum/view/img/abs.png"));
			int confirmation = 0;
			if(state) {
						...
						if(iNomTextInput.getText().isEmpty()    || 
						   ...
						   (!a1Checkbox.isSelected() && !a2Checkbox.isSelected() && !a3Checkbox.isSelected() && !a4Checkbox.isSelected())) {
							...
							try {
								...
								Image acceptedAtelier = Image.load(ForumSupMTIOujdaView.class.getResource(acceptedPath));
								Image canceledAtelier = Image.load(ForumSupMTIOujdaView.class.getResource(canceledPath));
	...
	private List<Map<String, Object>> getRTableData() {
		...
		try {
			for(int i=0; i<jRTableData.size(); i++) {
				...
				mapData.put("a1", Image.load(ForumSupMTIOujdaView.class.getResource(jMapData.get("a1").toString())));
				mapData.put("a2", Image.load(ForumSupMTIOujdaView.class.getResource(jMapData.get("a2").toString())));
				mapData.put("a3", Image.load(ForumSupMTIOujdaView.class.getResource(jMapData.get("a3").toString())));
				mapData.put("a4", Image.load(ForumSupMTIOujdaView.class.getResource(jMapData.get("a4").toString())));
	...
	private void saveRechercheData() {
		try {
			...
			a1.setId("a1");
			a1.setName("Atelier 1 : Entreprenariat et création d'entreprise");
